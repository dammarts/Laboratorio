            ...
            <table style={s.table}>
              ...
              <tbody>
                ...
                {reservas.map((r) => {
                  ...
                  const abierto = motivoId === r.reserva_id
                  return (
                    <>
                      <tr key={r.reserva_id} style={s.tr}>
                        ...
                        <td style={s.td}>
                          {r.estado === 'ACTIVA' && (
                            <button
                              style={s.btnCancelar}
                              onClick={() => { setMotivoId(abierto ? null : r.reserva_id); setMotivo('') }}
                            >
                      ...
                      {abierto && (
                        <tr key={`motivo-${r.reserva_id}`}>
                          <td colSpan={6} style={s.motivoRow}>
                            <div style={{ display: 'flex', gap: 8 }}>
                              ...
                              />
                              <button style={s.btnConfirm} onClick={() => handleCancelar(r.reserva_id)}>
                                Confirmar
                              </button>
                              <button style={s.btnClose} onClick={() => { setMotivoId(null); setMotivo('') }}>
                                ✕
                              </button>
                            </div>
                          </td>
...
const s = {
  ...
  motivoRow: { padding: '10px 16px', background: '#fef9f9', borderBottom: '1px solid #f1f5f9' },
  input: { padding: '7px 12px', border: '1px solid #ddd', borderRadius: 8, fontSize: 13 },
  btnConfirm: { background: '#dc2626', color: '#fff', border: 'none', padding: '7px 16px', borderRadius: 8, fontSize: 13, cursor: 'pointer' },
