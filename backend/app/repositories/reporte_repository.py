...
def uso_por_laboratorio(db: Session, filtros: FiltrosReporte) -> List[dict]:
    query = (
        db.query(
            ...
            (func.coalesce(
                func.sum(
                    case([(Reserva.estado != EstadoReserva.CANCELADA.value,
                           func.datediff(text("minute"), Reserva.hora_inicio, Reserva.hora_fin))],
                         else_=0)
                ), 0
            ) / 60.0).label("horas_ocupadas"),
            func.sum(
                case([(Reserva.estado == EstadoReserva.CANCELADA.value, 1)], else_=0)
    ...
    if filtros.laboratorio_id is not None:
        query = query.filter(Laboratorio.laboratorio_id == filtros.laboratorio_id)
    if filtros.usuario_id is not None:
        query = query.filter(Reserva.usuario_creador_id == filtros.usuario_id)
...
def ocupacion_mensual(db: Session, mes: int, anio: int) -> List[dict]:
    rows = (
        db.query(
            ...
            func.count(Reserva.reserva_id).label("total_reservas"),
            (func.coalesce(
                func.sum(func.datediff(text("minute"), Reserva.hora_inicio, Reserva.hora_fin)),
                0
            ) / 60.0).label("horas_ocupadas"),
        )
        .join(Reserva, Reserva.laboratorio_id == Laboratorio.laboratorio_id)
...
def por_docente(db: Session, filtros: FiltrosReporte) -> List[dict]:
    query = (
        db.query(
            ...
            Usuario.email,
            func.count(
                case([(Reserva.estado != EstadoReserva.CANCELADA.value, Reserva.reserva_id)],
                     else_=None)
            ).label("total_reservas"),
            func.count(distinct(Reserva.laboratorio_id)).label("laboratorios_usados"),
            func.max(Reserva.fecha).label("ultima_reserva"),
        )
        .join(Reserva, Reserva.usuario_creador_id == Usuario.usuario_id)
        .filter(Usuario.rol == "DOCENTE")
        .group_by(Usuario.usuario_id, Usuario.email)
    )
    if filtros.usuario_id is not None:
        query = query.filter(Usuario.usuario_id == filtros.usuario_id)
    if filtros.laboratorio_id is not None:
